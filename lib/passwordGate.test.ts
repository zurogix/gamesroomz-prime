...
describe("temporary-password gate", () => {
  it.each([CHANGE_PASSWORD_PATH, `${CHANGE_PASSWORD_PATH}/`, "/login", "/api/me", "/api/me/password"])("allows %s", (path) => {
  ...
  it.each(["/api/games", "/api/games/1/assessment", "/api/team", "/api/me/other", "/api/me/password-changed", "/api/me/password/extra", "/api/team/1/reset-password"])("refuses the API route %s", (path) => {
